...
const SymblTopicTagCloud = () => {
    ...
    const [preTopicData,setPreTopicData ]=useState([{"id":"6345234321","value":"Deep learning","count":0.2,"messageIds":["5089854032688320"],"sentiment":{"polarity":{"score":0.1},"suggested":"neutral"}}]);
    ...
    let cv=window.localStorage.getItem("conversationId");
        ...
        return () => {
            ...
            return (
                <div style={{padding: "20px",backgroundColor:"white"}}>
                    <div style={{backgroundColor: "rgba(30, 164, 253, 0.1)",padding:"10px",color: "rgba(0, 0, 0, 1)"}}>
                <TagCloud
                    minSize={12}
                    maxSize={35}
                    padding={20}
...
const customRenderer = (tag, size, color) => {
    if(tag!=undefined&&tag.sentiment!=undefined&&tag.sentiment.suggested!=undefined) {
        const getSentimentColor = (str) => {
            if (str === 'negative') return '#FF7F7F' // red
            if (str === 'positive') return '#90ee90' // green
            return 'grey'
        }
        ...
        const sentimentTooltip = (
            <>
                <Typography display='block' variant="caption" align='center' ><b>{`${tag.value}`}</b></Typography>
                <Divider light style={{marginBottom: '6px'}}/>
                <div style={{display: 'flex', justifyContent: 'center'}}>
                    <Typography variant="caption">Sentiment:&nbsp;</Typography>
                    <Typography variant="caption"
                                style={{color: getSentimentColor(tag.sentiment.suggested)}}><b>{tag.sentiment.suggested}</b></Typography>
                </div>
                <div style={{display: 'flex', justifyContent: 'center'}}>
                    <Typography variant="caption">Score:&nbsp;</Typography>
                    <Typography variant="caption"><b>{`${tag.sentiment.polarity.score.toFixed(2)}`}</b></Typography>
                </div>
                <div style={{...styles.gradientBar}}>
                    <div style={{...styles.marker, left: `${getMarkerPosition(tag.sentiment.polarity.score)}%`}}/>
                </div>
            </>
        )
